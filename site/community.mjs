...
const ICONS = await fetch("../icons.json").then((r) => r.json()).catch(() => ({}));
...
export function runCard(run) {
  const a = el("a", "panel run"); a.href = `../run/?id=${encodeURIComponent(run.id)}`;
  const vs = el("div", "vs");
  const sideEl = (m, cls) => {
  ...
  const votes = el("span", "votes", "· · ·"); foot.appendChild(votes);
  a.appendChild(foot);
  verifyRun(run).then((v) => foot.replaceChild(badge(v.status, v.spanMs), b)).catch(() => {});
  getVotes(run.id).then((v) => { votes.textContent = `▲ ${v.up}   ▼ ${v.down}`; }).catch(() => {});
  return a;
}
